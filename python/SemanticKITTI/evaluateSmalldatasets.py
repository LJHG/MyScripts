...
# os.system("python evaluate_iou.py -d ../../../smalldatasets/smalldataset10-30 -p ./logs-v2-crf-smalldatasets/logs-v2-crf-10-30 --split valid")
'''
这个脚本用来evaluate 所有smalldatasets的IOU，采用的做法是把 evaluate_iou.py这个文件中的一些操作wrap一下
'''
import argparse
import os
import yaml
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser("./evaluateSmalldatasets.py")
  parser.add_argument(
      '--dataset', '-d',
      type=str,
      required=False,
  ...
  for index in range(406):
    FLAGS.dataset = "../../../smalldatasets/smalldataset{}-{}".format(index*10,index*10+20)
    FLAGS.predictions = "./logs-v2-crf-smalldatasets/logs-v2-crf-{}-{}".format(index*10,index*10+20)
    ...
    car_IOU = class_jaccard[1]
    bicycle_IOU = class_jaccard[2]
    person_IOU = class_jaccard[6]

    # 计算和官方数据的差值
    cur_dis = (car_IOU-0.818)**2 + (bicycle_IOU-0.185)**2 + (person_IOU-0.201)**2
    if(cur_dis < min_dis):
      min_dis = cur_dis
      min_index = index
  print("#######################")
  print("与官方数据差值距离最小为{},来自的index为{},来自{}-{}帧".format(min_dis,min_index,min_index*10,min_index*10+20))
